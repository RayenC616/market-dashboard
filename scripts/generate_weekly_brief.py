...
every Monday at 07:00 KST.

1. Computes the past week's stats for all 14 tracked assets from
   data/market_history.xlsx, plus the S&P 500's best/worst weekly movers.
2. Calls the Anthropic API (with web search) to research the week's real
   news and write the brief's narrative sections in Korean, grounded in
   the computed numbers.
3. Renders the result into brief.html (same template/style as the site),
   archives the previous brief.html into archive/, and updates archive.html.

Requires an ANTHROPIC_API_KEY repository secret. See README.md for setup.
"""
import json
import re
...
def fetch_weekly_movers(tickers, top_n=3):
    results = []
    for i in range(0, len(tickers), 20):
        ...
        for item in data.get("spark", {}).get("result", []):
            ...
            try:
                ...
                if len(closes) < 6:
                    continue
                last, base = closes[-1], closes[-6]
                change = pct(last, base)
                if change is not None:
                    results.append({"symbol": symbol, "name": name, "last": last, "pct1W": change})
            except Exception:
                continue

    results.sort(key=lambda r: r["pct1W"], reverse=True)
...
WEEKLY_BRIEF_TOOL = {
    ...
    "description": "Submit the completed weekly market brief content in Korean.",
    "input_schema": {
        "type": "object",
        "properties": {
            "headline": {"type": "string", "description": "짧은 헤드라인 (아카이브 목록에 쓰일 한 문장 요약)"},
            "executive_summary": {"type": "string", "description": "5~8문장 분량의 주간 요약 (한국어, 실제 수치와 근거를 포함)"},
            "drivers": {
                "type": "array",
                "items": {
                    "type": "object",
                ...
                "description": "이번 주 주요 변동 요인 3~4개, 각각 제목과 분석 문단(한국어)",
            },
            "best_stocks_commentary": {
                ...
                "description": "제공된 주간 최고 상승 종목 3개에 대한 한국어 코멘트",
            },
            "worst_stocks_commentary": {
                ...
                "description": "제공된 주간 최저 상승(최대 하락) 종목 3개에 대한 한국어 코멘트",
            },
            "sources": {
                "type": "array",
                "items": {
...
def call_claude(week_label, stats, best, worst):
    ...
    best_lines = "\n".join(f"- {s['symbol']} ({s['name']}): 주간 {fmt_pct(s['pct1W'])}, 현재가 {s['last']:.2f}" for s in best)
    worst_lines = "\n".join(f"- {s['symbol']} ({s['name']}): 주간 {fmt_pct(s['pct1W'])}, 현재가 {s['last']:.2f}" for s in worst)

    prompt = f"""당신은 기관 투자자용 주간 시황 브리프를 작성하는 애널리스트입니다.
대상 주간: {week_label}
...
[S&P500 주간 최고 상승 종목 3개]
{best_lines}

[S&P500 주간 최저 상승(최대 하락) 종목 3개]
{worst_lines}

작업 지시:
1. 웹 검색 도구를 사용해 이번 주({week_label}) 실제 시장 뉴스(반도체, 금리, 지정학, 원자재 등)를 조사하세요.
2. 조사한 내용과 위 수치를 근거로 submit_weekly_brief 도구를 호출해 다음을 한국어로 작성하세요:
   - headline: 아카이브 목록용 한 줄 요약
   - executive_summary: 5~8문장, 위 수치를 인용하며 이번 주 시장을 종합 요약
   - drivers: 이번 주 시장을 움직인 핵심 요인 3~4개, 각각 제목과 분석(왜 시장에 영향을 미쳤는지)
...
3. 반드시 submit_weekly_brief 도구 호출로 최종 결과를 제출하세요."""
    ...
    tools = [{"type": "web_search_20250305", "name": "web_search"}, WEEKLY_BRIEF_TOOL]
    ...
    for _ in range(6):
        ...
        submit_call = next((b for b in response.content if b.type == "tool_use" and b.name == "submit_weekly_brief"), None)
        if submit_call:
            return submit_call.input

        if response.stop_reason in ("end_turn", "max_tokens"):
            messages.append({"role": "user", "content": "submit_weekly_brief 도구를 호출해 최종 브리프를 제출하세요."})
            continue

    raise RuntimeError("Claude did not submit a brief via submit_weekly_brief within the turn limit.")
...
def render_stock_cards(entries, commentary_by_symbol, sign_class):
    ...
    for e in entries:
        ...
        <div class="stock-row"><span class="stock-ticker">{esc(e["symbol"])}</span><span class="stock-chg {sign_class}">{fmt_pct(e["pct1W"])}</span></div>
...
def render_brief(week_label, session_date, prepared_date, stats, best, worst, llm, chart_data):
    ...
    template = (ROOT / "scripts" / "brief_template.html").read_text(encoding="utf-8")
    html = template
...
def archive_previous_brief(new_session_date):
    ...
    if meta_match:
        ...
        # Bootstrap case: archiving the hand-written brief.html that predates this script.
    ...
    if ARCHIVE_INDEX_PATH.exists():
        ...
        new_entry = f'''  <a class="entry" href="archive/{archived_name}">
    <div>
...
def main():
    ...
    week_label = f"{week_start.strftime('%Y-%m-%d')} ~ {last_date.strftime('%Y-%m-%d')}"
    session_date = last_date.date().isoformat()
    prepared_date = datetime.now(timezone.utc).date().isoformat()

    tickers = fetch_sp500_tickers()
    best, worst = fetch_weekly_movers(tickers)

    llm = call_claude(week_label, stats, best, worst)
    chart_data = build_ytd_chart_data(dates, series_by_asset)

    archive_previous_brief(session_date)

    new_html = render_brief(week_label, session_date, prepared_date, stats, best, worst, llm, chart_data)
    BRIEF_PATH.write_text(new_html, encoding="utf-8")

    print(f"Generated weekly brief for {week_label}, session_date={session_date}")
